...
const XP_FOR = { commit: 25, ai: 8, files: 3 };
const COINS_FOR = { commit: 10, ai: 3, files: 1 }; // 🪙 earned alongside XP
...
function award(type) {
  ...
  const gained = XP_FOR[type] || 0;
  const xp = prev.xp + gained;
  const level = levelFor(xp);
  const leveledUp = level > (prev.level || 1);
  const gainedCoins = (COINS_FOR[type] || 0) + (leveledUp ? LEVELUP_BONUS : 0);
